...
(function ativarPortuguesPerfeito() {
  ...
  if (typeof origIrPara === 'function') {
    window.irPara = function(...args) {
      ...
      setTimeout(() => { aplicarSpellcheck(); aplicarAutocorrecao(); if (window.itapValidacao) window.itapValidacao.init(); }, 300);
      return r;
    };
  }
  ...
  if (typeof _origSalvarArquivo === 'function') {
    window.salvarArquivo = async function(...args) {
      ...
      if (resultado) { marcarSalvo(); if (window.itapValidacao) window.itapValidacao.limparAlterados(); }
      return resultado;
    };
  }
