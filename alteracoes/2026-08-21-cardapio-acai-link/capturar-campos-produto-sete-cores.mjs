...
async function capture(name, width, height, mobile) {
  const port = mobile ? 9243 : 9244;
  const chrome = spawn('chromium', ['--headless=new', '--no-sandbox', '--disable-gpu', '--disable-dev-shm-usage', `--remote-debugging-port=${port}`, `--user-data-dir=/tmp/itap-campos-${name}-${Date.now()}`, `--window-size=${width},${height}`, 'about:blank'], { detached: true, stdio: 'ignore' });
  chrome.unref();
  let page;
  ...
  if (!page) throw new Error('Navegador de teste não iniciou.');
  ...
  await command('Page.navigate', { url: 'http://127.0.0.1:4173/retirada.html?campos-cores=1' });
  await wait(4500);
  await command('Runtime.evaluate', { expression: "document.querySelector('#sec-sorvetes-de-massa')?.scrollIntoView({block:'start'})" });
  await wait(500);
  const screenshot = await command('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false });
  writeFileSync(`${root}/demonstracao-${name}-campos-sete-cores.png`, Buffer.from(screenshot.data, 'base64'));
  socket.close(); process.kill(-chrome.pid, 'SIGTERM');
}

await capture('mobile', 375, 812, true);
await capture('desktop', 1280, 720, false);
console.log('Capturas de campos coloridos criadas.');
