...
function toggleAcc(id) {
  ...
  if (isOpen) {
    // Fecha: trava na altura real → anima até 0 (suave, sem salto)
    ...
    // Re-renderiza ao abrir para garantir dados carregados
    if (id === 'acc-iso') renderIso();
    if (id === 'acc-sobremesas') renderSobremesas();
    // Abre: scrollHeight para animação suave → após 500ms remove inline e deixa CSS (max-height:9999px)
    // assumir o controle — conteúdo dinâmico (chips, sabores) cresce livremente sem novo JS
    var body = el.querySelector('.acc-body');
    if (body) {
      body.style.maxHeight = body.scrollHeight + 'px';
      setTimeout(function() {
        if (el.classList.contains('open')) body.style.maxHeight = '';
      }, 500);
    }
    // Estabilidade: não mover a página ao abrir uma categoria.
    // O usuário controla a navegação; somente os botões de Voltar alteram o nível.
  }
}
...
function voltarCardapio(accId) {
  ...
  // 1. Fecha o accordion clicado sem deslocar a página
  if (accId) {
  ...
  // 2. Garante que o vc-container permanece aberto (não fecha o cardápio)
  ...
  // 3. Libera o accordion e volta à posição exata de onde ele foi aberto.
...
function renderSorvetes(){
  ...
  if(produtos&&produtos.sorvetes&&produtos.sorvetes.preços){
    Object.entries(produtos.sorvetes.preços).forEach(([key,preços])=>{
      ...
      c.innerHTML=imgCard(chave,'')+`<div class="prod-card-body"><div class="prod-nome">${nomes[key]||key}</div><div class="prod-desc">${desc[key]||''}</div><div class="prod-preço">A partir de R$ ${preço.toFixed(2).replace('.',',')}</div></div>`;
      g.appendChild(c);
    });
  }
...
function renderMilk(){
  ...
  if(produtos&&produtos.milkshake){
    ...
    tipos.forEach(t=>{
      const c=document.createElement('div');c.className='prod-card';
      c.innerHTML=imgCard(t.chave,'')+`<div class="prod-card-body"><div class="prod-nome">${t.nome}</div><div class="prod-preço">R$ ${t.preço.toFixed(2).replace('.',',')}</div></div>`;
      g.appendChild(c);
    });
  }
...
function renderTacas(){
  ...
  if(produtos&&produtos.tacas&&produtos.tacas.tradicionais){
    Object.entries(produtos.tacas.tradicionais).forEach(([nome,preço])=>{
      ...
      const c=document.createElement('div');c.className='prod-card';
      c.innerHTML=imgCard(chave,'')+`<div class="prod-card-body"><div class="prod-nome">Taça ${nome}</div><div class="prod-preço">R$ ${preço.toFixed(2).replace('.',',')}</div></div>`;
      g.appendChild(c);
    });
  }
...
function renderTacasP(){
  ...
  if(produtos&&produtos.tacas&&produtos.tacas.sujas){
    Object.entries(produtos.tacas.sujas).forEach(([nome,preço])=>{
      ...
      const c=document.createElement('div');c.className='prod-card';
      c.innerHTML=imgCard(chave,'')+`<div class="prod-card-body"><div class="prod-nome">Taça Suja – ${nome}</div><div class="prod-preço">R$ ${preço.toFixed(2).replace('.',',')}</div></div>`;
      g.appendChild(c);
    });
  }
...
function renderIso(){
  ...
  if(produtos&&produtos.isopores_viagem){
    ...
    Object.entries(produtos.isopores_viagem).forEach(([tam,preço])=>{
      ...
      d.innerHTML=imgCard(chave,'')+`<div class="prod-card-body"><div class="prod-nome">Isopore ${tam}</div><div class="prod-desc">Leve sorvete para casa</div><div class="prod-preço">R$ ${preço.toFixed(2).replace('.',',')}</div></div>`;
      grid.appendChild(d);
    });
    b.appendChild(grid);
...
function renderSobremesas(){
  ...
  if(produtos&&produtos.sobremesas){
    ...
    Object.entries(produtos.sobremesas).forEach(([nome,preço])=>{
      ...
      const emojiNome='';
      const d=document.createElement('div');d.className='prod-card';
      d.innerHTML=imgCard(chave,'')+`<div class="prod-card-body"><div class="prod-nome">${nome}</div><div class="prod-preço">R$ ${preço.toFixed(2).replace('.',',')}</div></div>`;
      grid.appendChild(d);
    });
    b.appendChild(grid);
    const btn=document.createElement('div');btn.style='margin-top:12px';
...
const RESPOSTAS={
  ...
  // ── CARDÁPIO E SABORES ──
  'sabor':'Temos 38 Sabores Tipo artesanal! Destaques: Chocolate, Nutella, Leite Ninho, Morango Trufado, Ferrero Rocher, Pistache, Kinder Ovo (choc. branco), Ovomaltine e muito mais. Veja o cardápio completo no site!',
  'sabores':'Temos 38 Sabores Tipo artesanal! Destaques: Chocolate, Nutella, Leite Ninho, Morango Trufado, Ferrero Rocher, Pistache, Kinder Ovo (choc. branco), Ovomaltine e muito mais. Veja o cardápio completo no site!',
...
function getComplementosCardápio() {
  const PADRAO = [
    {id:'canudinho',nome:'Canudinho Wafer UN',preço:0.25,estoque:999},
    {id:'casquinha',nome:'Casquinhas UN',preço:0.25,estoque:999},
    {id:'cascão',nome:'Cascão UN',preço:1.00,estoque:999},
    {id:'cestinha',nome:'Cestinha UN',preço:1.00,estoque:999},
    {id:'cobertura',nome:'Cobertura 1.3L UN',preço:40.00,estoque:999}
  ];
...
function renderComplementosCardápio() {
  ...
  if (!el) return;
  const comps = getComplementosCardápio();
  let fotos = {};
  ...
  el.innerHTML = `
    <div style="padding:4px 0 10px">
      ${comps.map(c => {
        ...
        const fotoHtml = foto
          ? `<img decoding="async" loading="lazy" src="${foto}" alt="Complemento Gourmet para Sorvete Tipo artesanal - Sorveteria Itapolitana" style="width:50px;height:50px;object-fit:cover;border-radius:10px;margin-right:12px;flex-shrink:0;" onerror="this.style.display='none'">`
          : `<div style="width:50px;height:50px;border-radius:10px;background:linear-gradient(135deg,#FFEBEE,#FFCDD2);display:flex;align-items:center;justify-content:center;font-size:24px;margin-right:12px;flex-shrink:0;">🍪</div>`;
        return `<div style="display:flex;align-items:center;background:${esgotado?'#FFF3F3':'#FAFAFA'};border:1.5px solid ${esgotado?'#FFCDD2':'#E0E0E0'};border-radius:12px;padding:12px;margin-bottom:10px;${esgotado?'opacity:0.6;':''}">
          ${fotoHtml}
          <div style="flex:1;">
            ...
            <div style="font-size:0.82rem;color:#B71C1C;font-weight:700;">R$ ${c.preço.toFixed(2).replace('.',',')} / un.</div>
          </div>
        </div>`;
      }).join('')}
      <div style="background:linear-gradient(135deg,#B71C1C,#E53935,#FF5252);color:#fff;border-radius:12px;padding:14px 18px;text-align:center;margin-top:4px">
        <div style="font-size:0.88rem;font-weight:700;margin-bottom:10px">🛒 Para encomendar complementos, acesse a página de Encomendas no menu acima</div>
        <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:8px">
          <a href="encomendas.html#acréscimos" style="display:inline-block;background:#fff;color:#B71C1C;border:none;border-radius:10px;padding:10px 22px;font-size:0.9rem;font-weight:900;cursor:pointer;text-decoration:none">Ver opções</a>
          <a href="${buildWhatsAppHref('Olá! Gostaria de consultar os acréscimos disponíveis para meu pedido.')}" target="_blank" rel="noopener" style="display:inline-block;background:linear-gradient(135deg,#25D366,#128C7E);color:#fff;border:none;border-radius:10px;padding:10px 22px;font-size:0.9rem;font-weight:900;cursor:pointer;text-decoration:none">Falar no WhatsApp</a>
        </div>
      </div>
    </div>`;
}
