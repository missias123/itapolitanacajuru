...
(function () {
  ...
  const state = { data: null, catalog: [], cart: loadCart(), flavorProduct: null, popsicleGroup: null, selectedFlavors: [], flavorCounts: {}, flavorPreferences: [], activeFlavorPreference: 0, popsiclePreferences: [], activePopsiclePreference: 0, popsicleQuantity: 1, boxAddOnCounts: {}, serviceMode: '', containerType: '', cakeChoice: '', query: '', lastCatalogSku: null };
  ...
  function needsFlavorPreferences(product) { const rule = flavorRule(product); return !product?.fixedAcai && !isIceCreamCake(product) && rule.source === 'massa' && rule.max > 0 && rule.max <= 3; }
  function preferenceCountForPopsicle(products) { return products?.[0]?.picole?.groupId === 'especiais' ? 1 : 3; }
  function preferenceReady(set, rule) { return Array.isArray(set) && set.length >= rule.min && set.length <= rule.max; }
  function preferenceText(preferences = []) { return preferences.map((set, index) => `Opção ${index + 1}: ${(set || []).map((item) => item.name || item).join(', ')}`).filter((line) => !line.endsWith(': ')).join(' | '); }
  ...
  function cartKey(product, flavors, serviceMode = '', containerType = '', cakeChoice = '', flavorDistribution = '', boxAddOns = [], flavorPreferences = []) { return `${product.sku}::${(flavors || []).map((item) => `${item.code || item}:${Number(item.quantity) || 1}`).sort().join('|')}::${serviceMode}::${containerType}::${cakeChoice}::${normalize(flavorDistribution).replace(/\s+/g, ' ')}::${(boxAddOns || []).map((item) => `${item.sku || item.id}:${Number(item.quantity) || 0}`).sort().join('|')}::${preferenceKey(flavorPreferences)}`; }
  ...
  function addProduct(product, flavors = [], serviceMode = '', openReview = true, containerType = '', cakeChoice = '', flavorDistribution = '', boxAddOns = [], flavorPreferences = []) {
    state.lastCatalogSku = product.sku;
    const key = cartKey(product, flavors, serviceMode, containerType, cakeChoice, flavorDistribution, boxAddOns, flavorPreferences);
  ...
  function buildProductList(products) {
    const list = document.createElement('div'); list.className = 'product-list'; products.forEach((product, index) => {
      ...
      const meta = isIceCreamCake(product) ? `Escolha 3 sabores · Retirada com antecedência mínima de 48 horas.` : hasFlavor ? flavorRule(product).label : 'Produto pronto para retirada';
  ...
  function renderFlavorGrid() {
    ...
    if (rule.distribution) {
      ...
      const requiresMode = needsPackagingChoice(product); const modeBox = $('#item-mode'); modeBox.hidden = !(ready && requiresMode); addOnsUi.section.hidden = !(ready && allowsBoxAddOns(product)); if (ready && allowsBoxAddOns(product)) renderBoxAddOns(); $$('[data-mode-choice]').forEach((choice) => choice.classList.toggle('is-selected', choice.dataset.modeChoice === state.serviceMode)); $$('input[name="item-mode"]').forEach((input) => { input.checked = input.value === state.serviceMode; });
  ...
  function renderCart() {
    ...
    state.cart.forEach((item) => {
      ...
      row.innerHTML = `<div class="cart-item__head"><div><p class="cart-item__name">${escape(displayName(item.name))}</p><p class="cart-item__meta">${escape([item.sku, item.size, container, flavors, flavorPreferences, flavorDistribution, selectedAddOns, fixedIngredients, includedExtras, cakeChoice].filter(Boolean).join(' · '))}</p></div><p class="cart-item__value">${money(itemTotal(item))}</p></div><div class="cart-item__pricing">${pricing.join('')}</div>`;
  ...
  function buildMessage(form) {
    ...
    state.cart.forEach((item, index) => {
      ...
      if (item.flavorPreferences?.length) item.flavorPreferences.forEach((set, preferenceIndex) => lines.push(`   Opção ${preferenceIndex + 1}: ${(set || []).map((flavor) => flavor.name || flavor).join(', ')}`));
