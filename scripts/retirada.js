...
(function () {
  ...
  function syncPickupDateConstraint() { const input = $('#pickup-date'); const field = $('#pickup-date-field'); const notice = $('#cake-pickup-rule'); const time = $('#pickup-time'); const timeHelp = $('#pickup-time-help'); if (!input || !field || !notice || !time || !timeHelp) return; const cakeProduction = hasCakeProductionLead(); field.hidden = !cakeProduction; notice.hidden = !cakeProduction; input.required = cakeProduction; if (cakeProduction) { input.min = brasiliaDateValue(new Date(Date.now() + 48 * 60 * 60 * 1000)); time.min = '11:00'; time.disabled = false; ensureFirstAvailablePickupTime('11:00'); timeHelp.textContent = '⏰ ESCOLHA A PARTIR DE 11:00. Encomenda de torta ou caixa grande: escolha data e horário pelo horário de Brasília; a data precisa estar pelo menos 48 horas à frente.'; } else { input.value = ''; input.min = ''; const deadline = commonPickupDeadline(); if (deadline.minutes <= 20 * 60) { time.min = deadline.time; time.disabled = false; ensureFirstAvailablePickupTime(deadline.time); timeHelp.textContent = `⏰ ESCOLHA A PARTIR DE ${deadline.time}. O preparo mínimo é de 1 hora pelo horário de Brasília.`; } else { time.value = ''; time.min = '20:00'; time.disabled = true; timeHelp.textContent = 'Hoje não há horário com 1 hora de antecedência até às 20h00. Volte no próximo horário de atendimento.'; } } syncPickupTimeValidation(); }
