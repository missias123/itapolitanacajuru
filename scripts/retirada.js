...
(function () {
  ...
  function validPickupTime(value) { return /^([01]\d|2[0-3]):[0-5]\d$/.test(value || '') && value >= '10:00' && value <= '20:59'; }
  ...
  function pickupTimeBaseHelpText() {
    ...
    return deadline.minutes <= (20 * 60 + 59)
      ? `⏰ ESCOLHA A PARTIR DE ${deadline.time}. Use formato 24h (sem AM/PM) e ajuste pelas setas.`
      : 'Após 19h59, não dá para abrir novo pedido de retirada hoje (limite de 1 hora até 20h59).';
  }
  function pickupTimeHelpText() { return hasCakeProductionLead() ? '⏰ ESCOLHA A PARTIR DE 09:00. Use formato 24h (sem AM/PM). Encomenda de torta ou caixa grande exige data com pelo menos 48 horas de antecedência.' : pickupTimeBaseHelpText(); }
  function syncPickupTimeStepper() {
    ...
    const baseMinutes = hasValue ? currentMinutes : minMinutes;
    const hasBase = Number.isFinite(baseMinutes);
    hour.textContent = hasBase ? pad2(Math.floor(baseMinutes / 60)) : '--';
    minute.textContent = hasBase ? pad2(baseMinutes % 60) : '--';
    $$('[data-pickup-adjust-unit]').forEach((button) => {
      ...
      const nextMinutes = baseMinutes + (delta * direction);
      const disabled = time.disabled || !hasBase || !Number.isFinite(minMinutes) || !Number.isFinite(maxMinutes) || nextMinutes < minMinutes || nextMinutes > maxMinutes;
  ...
  function syncPickupTimeHelp(baseText) {
    ...
    if (!help) return;
    const convertedFrom = time?.dataset?.convertedFrom || '';
    if (convertedFrom) {
      help.textContent = `${baseText} Horário AM (${convertedFrom}) não é permitido para retirada e foi ajustado para ${time.value} (PM).`;
      return;
    }
    help.textContent = baseText;
  }
  function normalizePickupTimeValue(value) {
  ...
  function normalizePickupTimeField() {
    ...
    const normalized = normalizePickupTimeValue(time.value);
    if (normalized && normalized !== time.value) {
  ...
  function syncPickupDateConstraint() { const input = $('#pickup-date'); const field = $('#pickup-date-field'); const notice = $('#cake-pickup-rule'); const time = $('#pickup-time'); const timeHelp = $('#pickup-time-help'); if (!input || !field || !notice || !time || !timeHelp) return; const cakeProduction = hasCakeProductionLead(); field.hidden = !cakeProduction; notice.hidden = !cakeProduction; input.required = cakeProduction; if (cakeProduction) { input.min = brasiliaDateValue(new Date(Date.now() + 48 * 60 * 60 * 1000)); time.min = '10:00'; time.max = '20:59'; time.disabled = false; ensureFirstAvailablePickupTime('10:00'); syncPickupTimeHelp(pickupTimeHelpText()); } else { input.value = ''; input.min = ''; time.max = '20:59'; const now = brasiliaParts(); const deadline = commonPickupDeadline(); if (now.minutes < 10 * 60) { time.value = ''; time.min = '10:00'; time.disabled = true; syncPickupTimeHelp('Pedidos de retirada liberam às 10h00 (Brasília).'); } else if (deadline.minutes <= (20 * 60 + 59)) { time.min = deadline.time; time.disabled = false; ensureFirstAvailablePickupTime(deadline.time); syncPickupTimeHelp(pickupTimeHelpText()); } else { time.value = ''; time.min = '20:59'; time.disabled = true; syncPickupTimeHelp('Após 19h59, não dá para abrir novo pedido de retirada hoje.'); } } normalizePickupTimeField(); syncPickupTimeHelp(pickupTimeHelpText()); syncPickupTimeStepper(); syncPickupTimeValidation(); }
  ...
  function submitOrder(event) { event.preventDefault(); const error = $('#form-error'); error.classList.remove('is-visible'); normalizePickupTimeField(); syncGuidedForm(); if (!formFlow.ready) return showFormError('Complete as etapas na ordem indicada antes de enviar.'); if (!retiradaAberta()) { window.ItapHorarioPedidos?.aviso('retirada'); return showFormError(window.ItapHorarioPedidos?.textoAviso('retirada') || 'Pedidos pelo site somente de segunda a sexta, das 10h00 às 20h00. A retirada começa às 11h00. Exceto sábados, domingos e feriados regionais de Cajuru/SP. Vá até nossa loja e peça pessoalmente.'); } const form = Object.fromEntries(new FormData(event.currentTarget).entries()); form.horario = normalizePickupTimeValue(form.horario); if (!state.cart.length) return showFormError('Escolha pelo menos um produto antes de enviar.'); if (!form.nome?.trim()) return showFormError('Informe o nome de quem vai retirar.'); if (!validatePhone(form.telefone || '')) return showFormError('Digite somente o número do celular após o DDD 16.'); if (!validPickupTime(form.horario)) return showFormError('Escolha um horário de retirada entre 10h00 e 20h59 (Brasília).'); if (hasCakeProductionLead()) { if (!form.data_retirada) return showFormError('Para encomenda de torta ou caixa grande, escolha a data desejada para retirar.'); if (!validCakeLeadTime(form.data_retirada, form.horario)) return showFormError('Encomendas de torta e caixa grande precisam de pelo menos 48 horas de antecedência pelo horário de Brasília.'); } else if (!validCommonLeadTime(form.horario)) { syncPickupTimeValidation(); $('#pickup-time')?.focus(); return showFormError(pickupTimeMessage()); } if (!form.aceite) return showFormError('Leia e marque o aceite das regras antes de enviar.'); setOrderStage(3); const text = buildMessage(form); window.open(`https://wa.me/${WHATSAPP}?text=${encodeURIComponent(text)}`, '_blank', 'noopener'); }
  ...
  $('#pickup-time').addEventListener('keydown', (event) => {
    ...
    if (event.key === 'ArrowUp') { event.preventDefault(); adjustPickupTime('minute', 1); return; }
    if (event.key === 'ArrowDown') { event.preventDefault(); adjustPickupTime('minute', -1); return; }
    if (event.key === 'PageUp') { event.preventDefault(); adjustPickupTime('hour', 1); return; }
    if (event.key === 'PageDown') { event.preventDefault(); adjustPickupTime('hour', -1); return; }
