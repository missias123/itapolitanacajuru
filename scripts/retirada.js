...
(function () {
  ...
  function addProduct(product, flavors = [], serviceMode = '', openReview = true, containerType = '') {
    ...
    const item = current || { key, sku: product.sku, name: product.name, size: product.size, category: product.category, type: product.type, price: product.price, flavors, serviceMode, containerType, packagingFee: serviceMode === 'travel' ? 1 : 0, quantity: 0, retail: product.picole?.varejo, wholesale: product.picole?.atacado, stock: product.picole?.stock };
    item.quantity += 1;
    setCart(item);
  ...
  function renderCart() {
    ...
    state.cart.forEach((item) => {
      ...
      const flavors = item.flavors?.length ? `Sabores: ${item.flavors.map((flavor) => flavor.name || flavor).join(', ')}` : ''; const container = item.containerType ? `Recipiente: ${item.containerType === 'casquinha' ? 'Casquinha' : 'Copo'}` : '';
      ...
      row.innerHTML = `<div class="cart-item__head"><div><p class="cart-item__name">${escape(displayName(item.name))}</p><p class="cart-item__meta">${escape([item.sku, item.size, container, flavors].filter(Boolean).join(' · '))}</p></div><p class="cart-item__value">${money(itemTotal(item))}</p></div><div class="cart-item__pricing">${pricing.join('')}</div>`;
