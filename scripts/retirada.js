...
(function () {
  ...
  const state = { data: null, catalog: [], cart: loadCart(), flavorProduct: null, popsicleGroup: null, selectedFlavors: [], flavorCounts: {}, flavorPreferences: [], activeFlavorPreference: 0, popsiclePreferences: [], activePopsiclePreference: 0, popsicleQuantity: 1, boxAddOnCounts: {}, acaiDoubleChoices: {}, includedCustomizationChoices: {}, serviceMode: '', containerType: '', cakeChoice: '', query: '', lastCatalogSku: null, lastCatalogViewport: null };
  ...
  function beginAcaiDouble(product) {
    state.flavorProduct = product; state.selectedFlavors = []; state.flavorCounts = {}; state.flavorPreferences = []; state.activeFlavorPreference = 0; state.boxAddOnCounts = {}; state.acaiDoubleChoices = {}; state.includedCustomizationChoices = {}; state.flavorDistribution = ''; state.serviceMode = ''; state.containerType = ''; state.cakeChoice = '';
  ...
  function buildProductList(products) {
    const list = document.createElement('div'); list.className = 'product-list'; products.forEach((product, index) => {
      ...
      const meta = isIceCreamCake(product) ? `Escolha 3 sabores · Retirada com antecedência mínima de 48 horas.` : hasFlavor ? flavorRule(product).label : hasAcaiDouble ? 'Escolha quais complementos do copo podem ser pedidos em dobro.' : 'Produto pronto para retirada';
      ...
      const button = document.createElement('button'); button.className = 'add-btn'; button.type = 'button'; const label = hasFlavor ? 'Escolher sabores' : hasAcaiDouble ? 'Escolher complementos' : 'Adicionar ao pedido'; applyOrderButtonState(button, label, Boolean(product.selectable && product.available));
      button.addEventListener('click', () => runWhenRetiradaOpen(() => { state.lastCatalogSku = product.sku; captureCatalogViewport(product.sku); if (hasFlavor) beginFlavors(product); else if (hasAcaiDouble) beginAcaiDouble(product); else addProduct(product); })); row.append(button); list.append(row);
    }); return list;
  }
