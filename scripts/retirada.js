...
(function () {
  ...
  function buildMessage(form) {
    ...
    state.cart.forEach((item, index) => {
      ...
      if (item.boxAddOns?.length) lines.push(`   Adicionais: ${item.boxAddOns.map((addOn) => `${addOn.quantity} ${addOn.name} · SKU ${addOn.sku} · ${money(Number(addOn.quantity) * Number(addOn.price))}`).join(' | ')}`);
      ...
      if (item.boxAddOns?.length) lines.push(`   Adicionais: ${money(itemAddOnTotal(item))}`);
