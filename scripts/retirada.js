...
(function () {
  ...
  function renderIncludedCustomizations(product, ready) {
    ...
    const selected = selectedIncludedCustomizations(); ui.total.textContent = selected.length ? `Incluídos no valor: ${selected.join(' · ')}` : 'Nenhuma cobertura ou complemento selecionado. O valor do produto não muda.';
  }
  ...
  function renderCart() {
    ...
    state.cart.forEach((item) => {
      ...
      if (item.boxAddOns?.length) pricing.push(`<span>Adicionais: ${money(itemAddOnTotal(item))}</span>`);
  ...
  function pickupTimeMessage() { const time = $('#pickup-time'); if (!time || hasCakeProductionLead() || !time.value) return ''; if (!validPickupTime(time.value)) return 'Escolha um horário entre 11h00 e 20h00 (segunda a sexta, exceto feriados).'; if (!validCommonLeadTime(time.value)) return `Pelo horário de Brasília, escolha a partir de ${commonPickupDeadline().time}. A retirada exige no mínimo 1 hora de antecedência.`; return ''; }
  ...
  function buildMessage(form) {
    const schedule = hasCakeProductionLead() ? `📅 Data desejada: ${form.data_retirada}` : '📅 Retirada: hoje (mínimo de 1h de antecedência em Brasília)';
    const lines = ['🍦 SOLICITAÇÃO DE RETIRADA — Itapolitana Cajuru', '', '👤 DADOS DO CLIENTE', `• Nome: ${form.nome}`, `• WhatsApp: ${phoneForMessage(form.telefone)}`, schedule, `🕒 Horário desejado: ${form.horario} (Brasília)`, `💳 Pagamento: ${form.pagamento}`, '', '🧾 RESUMO DO PEDIDO', `• Total dos produtos: ${money(totalProducts())}`, `• Total de adicionais: ${money(totalAddOns())}`, `• Total de embalagens: ${money(totalPackaging())}`, `• TOTAL FINAL: ${money(total())}`, `• Observações: ${form.observacoes || 'Nenhuma'}`, '', '🍨 ITENS'];
    state.cart.forEach((item, index) => {
      lines.push(`${index + 1}) ${item.name}${item.size ? ` — ${item.size}` : ''}`);
      lines.push(`• Código: ${item.sku} | Quantidade: ${item.quantity}`);
      ...
      if (item.boxAddOns?.length) lines.push(`• Total adicionais: ${money(itemAddOnTotal(item))}`);
      if (item.serviceMode === 'travel') lines.push(`• Embalagem viagem: ${item.packagingName || 'Embalagem para viagem'} (${item.packagingSku || 'EMB-VIAGEM'}) ${item.packagingIncluded ? '- incluída' : `- ${money(itemPackagingTotal(item))}`}`);
      if (item.serviceMode === 'store') lines.push('• Consumo na loja: sem taxa de embalagem');
      lines.push(`• Subtotal: ${money(itemTotal(item))}`, '');
    });
    lines.push('⚠️ STATUS E REGRAS', '• AGUARDANDO LIGAÇÃO E CONFIRMAÇÃO HUMANA.', '• Sem ligação do cliente ou resposta humana em até 15 minutos: considerar CANCELADA.', '• Não separar, elaborar ou iniciar produção antes da conferência por ligação.', '', '✅ ACEITE DO CLIENTE', 'Cliente declarou que leu e aceitou a ligação de confirmação, o prazo de 15 minutos e a regra de que mudanças só valem após confirmação humana.');
    return lines.join('\n');
  }
  function submitOrder(event) { event.preventDefault(); const error = $('#form-error'); error.classList.remove('is-visible'); syncGuidedForm(); if (!formFlow.ready) return showFormError('Complete as etapas na ordem indicada antes de enviar.'); if (!retiradaAberta()) { window.ItapHorarioPedidos?.aviso('retirada'); return showFormError(window.ItapHorarioPedidos?.textoAviso('retirada') || 'Pedidos para retirada disponíveis de segunda a sexta, das 11h00 às 20h00, exceto feriados.'); } const form = Object.fromEntries(new FormData(event.currentTarget).entries()); if (!state.cart.length) return showFormError('Escolha pelo menos um produto antes de enviar.'); if (!form.nome?.trim()) return showFormError('Informe o nome de quem vai retirar.'); if (!validatePhone(form.telefone || '')) return showFormError('Digite somente o número do celular após o DDD 16.'); if (!validPickupTime(form.horario)) return showFormError('Escolha um horário de retirada entre 11h00 e 20h00 (segunda a sexta, exceto feriados).'); if (hasCakeProductionLead()) { if (!form.data_retirada) return showFormError('Para encomenda de torta ou caixa grande, escolha a data desejada para retirar.'); if (!validCakeLeadTime(form.data_retirada, form.horario)) return showFormError('Encomendas de torta e caixa grande precisam de pelo menos 48 horas de antecedência pelo horário de Brasília.'); } else if (!validCommonLeadTime(form.horario)) { syncPickupTimeValidation(); $('#pickup-time')?.focus(); return showFormError(pickupTimeMessage()); } if (!form.aceite) return showFormError('Leia e marque o aceite das regras antes de enviar.'); setOrderStage(3); const text = buildMessage(form); window.open(`https://wa.me/${WHATSAPP}?text=${encodeURIComponent(text)}`, '_blank', 'noopener'); }
