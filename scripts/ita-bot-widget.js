...
(function () {
  ...
  function _itabotInjetarCss() {
    ...
    style.textContent = [
      ...
      '#itabot-launcher { position:fixed; right:calc(12px + env(safe-area-inset-right, 0px)); bottom:calc(14px + env(safe-area-inset-bottom, 0px)); z-index:2147482000; display:flex; flex-direction:column; align-items:center; justify-content:flex-start; gap:0; width:82px; height:94px; max-width:calc(100vw - 20px); padding:0; border:0; border-radius:0; color:#07579c; background:transparent; box-shadow:none; cursor:pointer; user-select:none; -webkit-tap-highlight-color:transparent; touch-action:manipulation; transition:transform .2s ease, opacity .2s ease, filter .2s ease, left .22s ease, right .22s ease, top .22s ease, bottom .22s ease; }',
      '#itabot-launcher:hover { transform:translateY(-3px) scale(1.04); filter:drop-shadow(0 0 10px rgba(34,194,255,.48)); }',
      '#itabot-launcher:focus-visible { outline:2px solid #22c2ff; outline-offset:4px; border-radius:16px; }',
      '#itabot-launcher.itabot-launcher-icon-only { width:82px; height:94px; padding:0; justify-content:flex-start; gap:0; }',
      '#itabot-launcher.itabot-launcher-icon-only .itabot-launcher-copy { display:flex; }',
      '.itabot-launcher-robot { position:relative; flex:0 0 62px; width:52px; height:62px; display:block; filter:drop-shadow(0 5px 5px rgba(2,56,104,.28)); animation:itabot-float 3.5s ease-in-out infinite; }',
      '.itabot-launcher-robot svg { display:block; width:100%; height:100%; overflow:visible; }',
      '.itabot-launcher-copy { min-width:0; display:flex; flex-direction:column; align-items:center; justify-content:center; line-height:1; padding:0; margin-top:2px; }',
      '.itabot-launcher-name { display:none; }',
      '.itabot-launcher-question { margin:0; font-size:11px; font-weight:950; letter-spacing:1.15px; text-transform:uppercase; color:#07579c; white-space:nowrap; text-shadow:0 0 5px rgba(34,194,255,.72),0 1px 1px rgba(255,255,255,.95); animation:itabot-question-pulse 1.8s ease-in-out infinite; transform-origin:center; }',
      ...
      '@media (max-width:600px) { #itabot-launcher { right:calc(10px + env(safe-area-inset-right, 0px)); bottom:calc(76px + env(safe-area-inset-bottom, 0px)); width:70px; height:82px; max-width:calc(100vw - 20px); padding:0; } .itabot-launcher.itabot-launcher-icon-only { width:70px; height:82px; } .itabot-launcher-robot { flex-basis:54px; width:45px; height:54px; } .itabot-launcher-question { font-size:10px; letter-spacing:.95px; } }',
    ...
    launcher.innerHTML = [
      '<span class="itabot-launcher-robot" aria-hidden="true">',
        '<svg viewBox="0 0 80 96" role="img" aria-hidden="true">',
          '<defs>',
            '<linearGradient id="itabotBody" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#d8f8ff"/><stop offset=".42" stop-color="#63c9f2"/><stop offset="1" stop-color="#1769b7"/></linearGradient>',
            '<linearGradient id="itabotHelmet" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#e8fcff"/><stop offset=".45" stop-color="#70d7f8"/><stop offset="1" stop-color="#1c70bd"/></linearGradient>',
            '<linearGradient id="itabotScreen" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#123653"/><stop offset=".5" stop-color="#071827"/><stop offset="1" stop-color="#020810"/></linearGradient>',
            ...
            '<filter id="blueOutline"><feMorphology operator="dilate" radius="1.5" in="SourceAlpha" result="dilated"/><feFlood flood-color="#16bfff" result="blue"/><feComposite in="blue" in2="dilated" operator="in" result="outline"/><feMerge><feMergeNode in="outline"/><feMergeNode in="SourceGraphic"/></feMerge></filter>',
            '<clipPath id="itabotTickerClip"><rect x="20" y="33.1" width="40" height="4.2" rx="1.4"/></clipPath>',
          '</defs>',
          '<g filter="url(#blueOutline)">',
            ...
            '<!-- Tela de TV LED: rosto e ticker vivem no mesmo visor -->',
            '<path d="M17 28c2-12 10-19 23-19s21 7 23 19v9H17Z" fill="url(#itabotScreen)" stroke="#63eaff" stroke-width=".9"/>',
            '<path d="M20 34h40" stroke="#1a5470" stroke-width=".65" opacity=".9"/>',
            '<!-- Olhos LED (^ ^) -->',
            '<path d="M27 23 q4 -5 8 0" fill="none" stroke="#7cffff" stroke-width="2.4" stroke-linecap="round" filter="url(#itabotGlow)"/>',
            '<path d="M45 23 q4 -5 8 0" fill="none" stroke="#7cffff" stroke-width="2.4" stroke-linecap="round" filter="url(#itabotGlow)"/>',
            '<!-- Boca LED expressiva -->',
            '<path d="M35 29 q5 5 10 0" fill="none" stroke="#7cffff" stroke-width="2" stroke-linecap="round" filter="url(#itabotGlow)"/>',
            '<!-- Mensagem passando na própria tela, sem apagar olhos e boca -->',
            '<g clip-path="url(#itabotTickerClip)">',
              '<text class="itabot-ticker-text" x="63" y="36.2" fill="#ffffff" font-size="3.1" font-family="Arial, sans-serif" font-weight="800" letter-spacing=".35">SORVETE • 38 SABORES • DÚVIDAS?</text>',
              '<text class="itabot-ticker-text" x="63" y="36.2" fill="#a9fbff" font-size="3.1" font-family="Arial, sans-serif" font-weight="800" letter-spacing=".35"><animate attributeName="x" from="63" to="-62" dur="6s" repeatCount="indefinite"/>SORVETE • 38 SABORES • DÚVIDAS?</text>',
            '</g>',
            '<!-- Braços -->',
            '<path d="M14 55 6 62M66 55l10 4" stroke="#55bfe9" stroke-width="5" stroke-linecap="round"/>',
            '<!-- Sorvete -->',
            ...
            '<!-- Pernas -->',
            '<path d="M30 82v6M50 82v6" stroke="#3a9ed8" stroke-width="6" stroke-linecap="round"/>',
          '</g>',
        '</svg>',
      '</span>',
